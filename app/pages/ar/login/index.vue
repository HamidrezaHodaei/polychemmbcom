  ...
  <div class="min-h-screen flex items-center justify-center bg-[#f1f2f2] p-6 font-sans relative text-[#808285]">
    ...
    <div class="mobile-bg md:hidden" aria-hidden="true">
      <div class="mobile-bg-frame">
        <img src="/login.webp" alt="Mobile background" class="mobile-bg-img" />
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg h-[640px] max-w-5xl w-full flex flex-col md:flex-row overflow-hidden md:h-[580px] relative form-card">
      ...
      <form @submit.prevent="handleLogin" class="w-full md:w-[420px] p-8 flex flex-col items-center justify-center h-full pt-0 relative">
        ...
        <!-- Back to Home -->
        <NuxtLink to="/" aria-label="Back to home" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 text-2xl leading-none">
        ...
        <div class="text-center mb-8">
          <h1 class="text-2xl font-semibold mb-2">Client Login</h1>
        ...
        <div class="w-full max-w-[420px] flex flex-col gap-6">
          ...
          <div class="relative">
            <input 
              ...
              placeholder=" " 
              class="peer w-full border-b-2 border-gray-300 focus:border-[#FFCD05] outline-none py-2 bg-white pr-10" 
              :disabled="loading"
        ...
        <div class="w-full max-w-[480px] mt-6 text-center text-sm text-gray-600 relative pb-10">
          <p>Need an account? <NuxtLink to="/en/contact" class="text-[#FFCD05] hover:text-[#808285]">Contact Us</NuxtLink></p>
        </div>
      </form>
    </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    errorMessage.value = error.data?.message || error.message || 'An error occurred'
  } finally {
    loading.value = false
  }
}
