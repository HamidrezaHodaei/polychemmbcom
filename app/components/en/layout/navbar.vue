<!-- /components/en/layout/navbar.vue -->
<template>
  <div>
      ...
      <div class="navbar-inner navbar-container">
        <div class="flex items-center h-16 md:h-20">
          ...
          <div class="flex items-center justify-start md:border-r md:border-l md:border-white pl-2 md:pl-8 pr-2 md:pr-8 h-full flex-shrink-0 w-auto md:w-[230px]">
              <NuxtLink to="/en" aria-label="Go to home" class="block">
...
<div class="hidden md:flex items-center justify-end h-full w-auto flex-shrink-0">
  <div class="relative h-full flex items-center border-l border-r border-white px-2" ref="langDropdownEn">
    ...
    >
      ...
      >
        <span class="text-xs text-yellow-400">EN</span>
        <span>English</span>
      </button>
      <button
        @click="() => { switchLang('tr'); langOpen = false }"
        class="w-full px-4 py-3 text-sm text-gray-700 hover:bg-yellow-50 hover:text-yellow-600 transition-colors flex items-center justify-between"
      >
        <span class="text-xs text-gray-400">TR</span>
        <span>Türkçe</span>
      </button>
    </div>
  </div>
  ...
  <NuxtLink to="/clientarea/login" class="nav-link text-[13px] font-medium px-6 h-full text-white relative overflow-hidden border-l border-r border-white flex items-center justify-center">
    <span class="relative z-10">Customer Login</span>
  </NuxtLink>
</div>
        ...
        <transition name="mobile-menu">
          <div v-if="mobileMenuOpen" class="md:hidden pb-4 bg-[#808285] max-h-[calc(100vh-64px)] overflow-y-auto">
            <div class="flex flex-col gap-2 pt-4 px-4">
              ...
              <NuxtLink to="/en" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">HOME</span>
              </NuxtLink>
              <NuxtLink to="/en/aboutus" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">ABOUT US</span>
              </NuxtLink>
              <NuxtLink to="/en/news" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">NEWS</span>
              </NuxtLink>
              <NuxtLink to="/en/careers" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">CAREERS</span>
              </NuxtLink>
              <NuxtLink to="/en/contact" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">CONTACT US</span>
              </NuxtLink>
              <div class="pt-3 mt-2">
                ...
                <NuxtLink to="/en/login" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded block w-full text-left relative overflow-hidden">
    ...
    <Chatbox v-if="route.path === '/' || route.path === '/en/aboutus'" />
...
// ─── تابع کمکی: ریست کامل حالت منو ───────────────────────────────────────
const forceResetMenu = () => {
  if (!process.client) return
  // overflow را همیشه ریست کن
  try { document.body.style.overflow = '' } catch (err) {}
  isMenuOpen.value = false
  productActive.value = false
  isOpeningMenu.value = false
  hoveredItem.value = null
  // اگه fullscreenMenu وجود داره، pointer-events رو هم ریست کن
  if (fullscreenMenu.value) {
    fullscreenMenu.value.style.pointerEvents = 'none'
  }
}
...
const useCases = [
  ...
  { 
   ...
   { 
    ...
    path: '/en/products/POLYFIL-1300-EWA',
    description: 'POLYFILL 1300 EWA is a Poly Ethylene-based filler masterbatch containing 82% calcium carbonate (CaCO₃). The POLYFILL 1300 EWA  features a high, very fine, treated CaCO₃ content, ensuring excellent dispersion within the final product. It is specifically designed for direct addition during the processing of polyolefins, including extrusion and injection molding.',
  ...
  { 
    ...
    path: '/en/products/SlIPCHEM-E-178',
  ...
  { 
    code: '', 
    title: 'RAFCOLOR 1560', 
    path: '/en/products/RAFCOLOR-1560',
  ...
  { 
    ...
    path: '/en/products/CALCICHEM-126-FP',
  ...
  { 
    ...
    path: '/en/products/CALCICHEM-110-FRF',
  ...
  { 
    ...
    path: '/en/products/CALCICHEM-275-PM',
  ...
  { 
    code: '', 
    title: 'UVCHEM MB-R18', 
    path: '/en/products/UVCHEM-MB-R18',
...
const isProductPage = (path) => {
  ...
  return normalizedPath.includes('/en/product') ||
         normalizedPath === '/en/products' ||
         normalizedPath.includes('/en/products/') ||
         normalizedPath.includes('/en/product_polychem') ||
         normalizedPath === '/en/product'
}
...
const handleReturnFromProduct = async () => {
  ...
  // Only auto-open menu on desktop (md breakpoint and up)
  const isMobile = () => window.innerWidth < 768
  
  if (isMainPage && !isMenuOpen.value && !isOpeningMenu.value && !isMobile()) {
    await nextTick()
    isMenuOpen.value = true
    await nextTick()
    try { hoveredItem.value = useCases[0] } catch (e) {}
...
// ─── Watch اصلی: جلوگیری از قفل شدن صفحه هنگام تغییر route ─────────────────
watch(() => route.path, async (newPath, oldPath) => {
  if (!newPath || newPath === oldPath) return

  // ① همیشه overflow را فوری ریست کن — قبل از هر چیز دیگه‌ای
  if (process.client) {
    try { document.body.style.overflow = '' } catch (err) {}
  }

  // ② اگه منو باز بود، فوری ببندش (بدون انتظار برای انیمیشن)
  if (isMenuOpen.value) {
    // kill هر انیمیشن در حال اجرا
    gsap.killTweensOf([
      menuContainer.value, menuTitle.value, closeButton.value,
      previewBox.value, previewImage.value, previewIcon.value,
      previewTitle.value, previewDesc.value,
      ...(menuItems.value || [])
    ])
    // فوری حالت رو ریست کن
    forceResetMenu()
  }

  // ③ بقیه منطق قبلی (برگشت از صفحه product)
  const prevPath = previousRoute.value?.path
  previousRoute.value = { path: newPath, query: route.query }

  if (!prevPath) return

  const comingFromProduct = isProductPage(prevPath)
...
const handleMenuItemClick = async (item) => {
  ...
  isMenuOpen.value = false
  productActive.value = false
  if (fullscreenMenu.value) fullscreenMenu.value.style.pointerEvents = 'none'
