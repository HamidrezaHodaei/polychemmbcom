    ...
    <div class="contact-launcher">
      ...
      <transition name="pill-fade">
        <button
          ...
          :aria-label="language === 'fa' ? 'تماس و پشتیبانی' : language === 'tr' ? 'Bizimle sohbet veya ara' : 'Chat or call us'"
        >
      ...
      <transition name="panel-slide">
        <div v-if="launcherPanelOpen" class="launcher-panel" role="dialog">
          <button class="panel-close-btn" @click="launcherPanelOpen = false" :aria-label="language === 'fa' ? 'بستن' : language === 'tr' ? 'Kapat' : 'Close'">
          ...
          <button class="panel-action-row" @click="handleChatNowFromLauncher">
            ...
            <span class="panel-action-label">{{ language === 'fa' ? 'چت آنلاین' : language === 'tr' ? 'Şimdi sohbet et' : 'Chat now' }}</span>
          </button>
          ...
          <button class="panel-action-row" @click="handleWhatsApp">
            ...
            <span class="panel-action-label">{{ language === 'fa' ? 'واتس‌اپ' : language === 'tr' ? 'WhatsApp' : 'WhatsApp' }}</span>
          </button>
          ...
          <div class="panel-call-group">
            <button class="panel-action-row" @click="handleCallSales">
              ...
              <span class="panel-action-label">{{ language === 'fa' ? 'تماس با فروش' : language === 'tr' ? 'Satış İletişim' : 'Call Sales' }}</span>
            </button>
    ...
    <div class="chat-window" :class="{ active: !minimized }" role="dialog" aria-label="POLYCHEM Sales Chat" v-cloak>
      ...
      <div class="chat-header">
        ...
        <div class="header-actions">
          <button class="header-btn" @click="clearChat" :title="language === 'fa' ? 'پاک کردن' : language === 'tr' ? 'Temizle' : 'Clear chat'">
          ...
          <button class="header-btn" @click="closeChat" :title="language === 'fa' ? 'کوچک کردن' : language === 'tr' ? 'Küçült' : 'Minimize'">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <transition name="quick-actions-fade">
          <div v-if="showQuickActions" class="quick-actions-wrapper">
            <p class="quick-actions-label">
              {{ language === 'fa' ? 'چند چیزی که می‌توانم کمک کنم:' : language === 'tr' ? 'Yardımcı olabileceğim birkaç şey:' : 'A few things I can help you with:' }}
            </p>
      ...
      <div class="chat-input-area">
        <input
          ...
          type="text"
          class="chat-input"
          :placeholder="language === 'fa' ? 'پیام خود را بنویسید...' : language === 'tr' ? 'Mesajınızı yazın...' : 'Type your message...'"
          @keypress.enter.prevent="sendMessage"
          :dir="language === 'fa' ? 'rtl' : 'ltr'"
        />
        <button class="send-btn" @click="sendMessage" :aria-label="language === 'fa' ? 'ارسال' : language === 'tr' ? 'Gönder' : 'Send'">
...
/* ─────────────────────────────────────────────────
   ...
   3 most important prompts drawn from existing patterns
───────────────────────────────────────────────── */
...
const messages = ref([
  {
    id: 1,
    sender: 'bot',
    text: `Merhaba! POLYCHEM'e hoş geldiniz. İleri teknoloji polimer bileşenler ve masterbatçlar konusunda uzmanız. Bugün size nasıl yardımcı olabilirim?`,
    lang: 'tr'
  }
])
const input = ref('')
...
/* ── Quick Actions visibility ──
   Show only before ANY user message has been sent.
   Hidden permanently once the user sends their first message. */
const userHasSentMessage = ref(false)
const showQuickActions = computed(() => !userHasSentMessage.value)
const currentQuickActions = computed(() => quickActionsConfig[language.value] || quickActionsConfig.en)
...
/* ─────────────────────────────────────────────────
   SEND QUICK ACTION
   Fires the prompt text directly, hides quick actions
───────────────────────────────────────────────── */
...
const dispatchMessage = async (text) => {
  ...
  // Mark that user has sent a message → hides quick actions
...
const clearChat = () => {
  userHasSentMessage.value = false   // ← reset so quick actions re-appear after clear
  messages.value = [{
    id: Date.now(), sender: 'bot',
    text: language.value === 'fa' ? 'سلام!  من اینجام برای کمک. چطور می‌تونم کمکتون کنم؟' : language.value === 'tr' ? 'Merhaba! Yardımcı olmak için buradayım. Sana nasıl yardımcı olabilirim?' : 'Hello!  I am here to assist. How can I help you?',
    lang: language.value
  }]
}
...
.panel-phone-sub { padding: 4px 0 2px 50px; }
.panel-local-link {
...
.message[dir="rtl"] .message-bubble { font-family: 'IRANYekan', 'Montserrat', sans-serif; }
...
.message.user[dir="rtl"] .message-bubble { font-family: 'IRANYekan', 'Montserrat', sans-serif; }
