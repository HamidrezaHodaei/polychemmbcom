<!-- /components/tr/layout/navbar.vue -->
<template>
  <div>
    <!-- Mobile Top Navbar (hidden on desktop) -->
    <NavbarMobTop class="lg:hidden" />

    <nav class="hidden lg:block bg-[#808285] fixed top-0 inset-x-0 z-50 lg:border-b" >
      <div class="navbar-inner navbar-container">
        <div class="flex items-center h-16 md:h-20">
          ...
          <div class="flex items-center justify-start md:border-r md:border-l md:border-white pl-2 md:pl-8 pr-2 md:pr-8 h-full flex-shrink-0 w-auto md:w-[230px]">
              <NuxtLink to="/tr" aria-label="Go to home" class="block">
          ...
          <div class="hidden md:flex items-center h-full md:border-r md:border-white">
            <button 
              ...
              :class="{ active: productActive || isProductPage(route.path) }"
            >
          ...
          <div class="hidden md:flex items-center flex-1 justify-center h-full">
            ...
            <NuxtLink 
              ...
              class="nav-link text-[13px] font-medium px-8 text-white relative overflow-hidden h-full flex items-center"
            >
              <span class="relative z-10">Haberler</span>
            </NuxtLink>
            ...
            <NuxtLink 
              ...
              class="nav-link text-[13px] font-medium px-8 text-white relative overflow-hidden h-full flex items-center"
            >
              <span class="relative z-10">Karıyer</span>
            </NuxtLink>
            ...
            >
              <span class="relative z-10">Iletişım
              </span>
            </NuxtLink>
          </div>

          <!-- Right Side -->
<div class="hidden md:flex items-center justify-end h-full w-auto flex-shrink-0">
  <div class="relative h-full flex items-center border-l border-r border-white px-2" ref="langDropdownEn">
    <button
      @click="langOpen = !langOpen"
      class="nav-link text-[13px] font-medium px-6 h-full text-white relative overflow-hidden"
    >
      <span class="relative z-10">TR</span>
    </button>
    <div
      v-show="langOpen"
      class="absolute top-full right-0 mt-1 w-36 bg-white rounded-lg shadow-lg z-[100] overflow-hidden border border-gray-100"
    >
      <button
        @click="() => { switchLang('ar'); langOpen = false }"
        class="w-full px-4 py-3 text-sm text-gray-700 hover:bg-yellow-50 hover:text-yellow-600 transition-colors flex items-center justify-between"
      >
        <span class="text-xs text-gray-400">AR</span>
        <span style="font-family: 'Tajawal', system-ui, -apple-system, sans-serif;">العربية</span>
      </button>
      <button
        @click="() => { switchLang('fa'); langOpen = false }"
        class="w-full px-4 py-3 text-sm text-gray-700 hover:bg-yellow-50 hover:text-yellow-600 transition-colors flex items-center justify-between"
      >
        <span class="text-xs text-gray-400">FA</span>
        <span style="font-family: 'IRANYekan', system-ui, -apple-system, sans-serif;">فارسی</span>
      </button>
      <button
        @click="() => { switchLang('en'); langOpen = false }"
        class="w-full px-4 py-3 text-sm text-gray-700 hover:bg-yellow-50 hover:text-yellow-600 transition-colors flex items-center justify-between"
      >
      ...
      <button
        class="w-full px-4 py-3 text-sm text-yellow-600 bg-yellow-50 flex items-center justify-between cursor-default"
      >
        <span class="text-xs text-yellow-400">TR</span>
        <span>Türkçe</span>
      </button>
    </div>
  </div>

  <NuxtLink to="/clientarea/login" class="nav-link text-[13px] font-medium px-6 h-full text-white relative overflow-hidden border-l border-r border-white flex items-center justify-center">
    <span class="relative z-10">Müşteri Girişi</span>
  </NuxtLink>
</div>
          ...
          <button 
            @click="toggleFullscreenMenu"
            class="md:hidden ml-auto text-white hover:text-yellow-400 px-4 py-3 rounded-lg hover:bg-black/20 transition-all duration-200 flex items-center justify-center"
        ...
        <transition name="mobile-menu">
          ...
          <div v-if="mobileMenuOpen" class="md:hidden pb-4 bg-[#808285] max-h-[calc(100vh-64px)] overflow-y-auto">
            <div class="flex flex-col gap-2 pt-4 px-4">
              ...
              <NuxtLink to="/tr" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">HOME</span>
              </NuxtLink>
              <NuxtLink to="/tr/aboutus" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">ABOUT US</span>
              </NuxtLink>
              <NuxtLink to="/tr/news" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">NEWS</span>
              </NuxtLink>
              <NuxtLink to="/tr/careers" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
                <span class="relative z-10">CAREERS</span>
              </NuxtLink>
              <NuxtLink to="/tr/contact" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded relative overflow-hidden w-full block">
              ...
              <div class="pt-3 mt-2">
                ...
                <NuxtLink to="/tr/login" class="nav-link-mobile text-[13px] font-medium text-white hover:text-gray-200 py-3 px-4 rounded block w-full text-left relative overflow-hidden">
    ...
    <div class="hidden lg:block h-16 lg:h-20"></div>

    <!-- Mobile bottom navbar (active on small screens only) -->
    <NavbarMob class="lg:hidden" />

    <!-- Render Chatbox on Home and About pages -->
    <Chatbox v-if="route.path === '/' || route.path === '/tr/aboutus'" />
      ...
      <div class="h-full flex flex-col md:flex-row">
        ...
        <div class="w-full md:w-[60%] bg-white relative flex flex-col h-full">
          ...
          >
            ...
            >
              <h2 class="text-3xl md:text-5xl font-bold text-gray-900 mb-2">Ürünümüz</h2>
...
import NavbarMobTop from './navbar-mob-top.vue'
...
const langDropdownEn = ref(null)

// Flag to prevent duplicate menu openings
const isOpeningMenu = ref(false)

// Track previous route for comparison
const previousRoute = ref(null)

// بسته شدن dropdown وقتی بیرونش کلیک میشه
const closeDropdownOutside = (e) => {
  if (process.client && langDropdownEn.value && !langDropdownEn.value.contains(e.target)) {
...
const useCases = [
  { 
    ...
    path: '/tr/products/0955W',
    description: 'Rotasyonel kalıplama uygulamaları için özel beyaz renkli plastik bileşik. Dar moleküler ağırlık dağılımına sahip lineer orta yoğunlukta polietilen kopolimer ve TiO2 içerir. Zorlu çevresel koşullara dayanıklı, güvenilir ve dayanıklı malzemeler için idealdir.',
    image: '/Product/Navbar/ROTOCHEM-0955W.webp',
    index: 0,
    offsetY: '15px',
    offsetTextY: '60px',
    offsetDescY: '130px'
  ...
  { 
    ...
    path: '/tr/products/0955B',
    description: 'Rotasyonel kalıplama uygulamaları için özel mavi renkli plastik bileşik. Dar moleküler ağırlık dağılımına sahip lineer orta yoğunlukta polietilen kopolimer ve TiO2 içerir. Tutarlı ve stabil malzemeler gerektiren endüstriyel ve tüketici uygulamaları için mükemmeldir.',
    image: '/Product/Navbar/ROTOCHEM-0955B.webp',
    index: 1,
    offsetY: '12px',
    offsetTextY: '4px',
    offsetDescY: '12px'
  ...
  { 
    code: '', 
    title: 'POLYFIL F700', 
    path: '/tr/products/POLYFIL-F700',
    description: 'HDPE şişirme film uygulamaları için yüksek performanslı polietilen bileşik. Üstün mekanik özellikler, mükemmel film homojenliği ve ultra ince film uygulamalarında (10-25 mikron) güvenilir işlenebilirlik sunacak şekilde tasarlanmıştır. Alışveriş çantaları, tişört torbaları, çöp torbaları ve gıda ile temas eden filmler için uygundur.',
    image: '/Product/Navbar/POLYFIL-F700.webp',
    index: 2,
    offsetY: '18px',
    offsetTextY: '6px',
    offsetDescY: '60px'
  ...
  { 
    code: '', 
    title: 'POLYFILL 1300 EWA', 
    path: '/tr/products/POLYFIL-1300-EWA',
    description: 'POLYFILL 1300 EWA, %82 kalsiyum karbonat (CaCO₃) içeren polietilen bazlı dolgu masterbatchtir. Çok ince ve işlenmiş CaCO₃ içeriği ile nihai üründe mükemmel dispersiyon sağlar. Ekstrüzyon ve enjeksiyon kalıplama dahil poliolefin işleme sırasında doğrudan ekleme için özel olarak tasarlanmıştır.',
  ...
  { 
    code: '', 
    title: 'HDCHEM 4760', 
    path: '/tr/products/HDCHEM-4760',
    description: 'Şişirme kalıplama uygulamaları için özel plastik bileşik. İyi akışkanlık, darbe dayanımı, ESCR ve rijitlik gibi avantajlı özelliklere sahip polietilen kopolimer bileşiği. Tutarlı ve stabil malzemeler için güvenilir bir seçim.',
    image: '/Product/Navbar/HDCHEM-4760.webp',
    index: 4,
    offsetY: '18px',
    offsetTextY: '6px',
    offsetDescY: '50px'    
  ...
  { 
    ...
    path: '/tr/products/SlIPCHEM-E-178',
    description: 'Polietilen taşıyıcı reçine içinde yüksek kaliteli slip ajanı ile yüksek performanslı slip masterbatch. Film sarımı, torba üretimi ve ambalajlama sırasında polimer film katmanları arasındaki sürtünme katsayısını (COF) azaltır. Mükemmel dispersiyon, yüksek termal stabilite ve tutarlı migrasyon performansı sunar.',
    image: '/Product/Navbar/SlIPCHEM-E178.webp',
    index: 5,
    offsetY: '18px',
    offsetTextY: '6px',
    offsetDescY: '50px' 
  ...
  { 
    code: '', 
    title: 'RAFCOLOR 1560', 
    path: '/tr/products/RAFCOLOR-1560',
    description: 'Yüksek oranda rutil titanyum dioksit ve termoplastik polipropilen reçine içeren beyaz masterbatch. Mükemmel dispersiyon ve termal stabiliteye sahip yüksek konsantrasyonlu beyaz MB. Rafia, bantlar, CF/BCF iplikler ve diğer ürünler için önerilir.',
    image: '/Product/Navbar/RAFCOLOR-1560.webp',
    index: 6,
    offsetDescY: '90px',
    offsetTextY: '50px',
    offsetY: '20px'
  ...
  { 
    ...
    path: '/tr/products/CALCICHEM-126-FP',
    description: '80% kalsiyum karbonat (CaCO₃) içeren polipropilen bazlı dolgu masterbatch. Çok ince ve işlenmiş CaCO₃ içeriği ile mükemmel dispersiyon sağlar. Ekstrüzyon ve enjeksiyon kalıplama dahil poliolefin işleme sırasında doğrudan ekleme için tasarlanmıştır.',
    image: '/Product/Navbar/CALCICHEM-126FP.webp',
    index: 7,
    offsetDescY: '50px',
    offsetTextY: '50px',
    offsetY: '20px'
  ...
  { 
    ...
    path: '/tr/products/CALCICHEM-110-FRF',
    description: 'Yüksek, çok ince işlenmiş CaCO₃ içeriğine sahip mineral modifiye edici. Film, rafya ve halatlar için tasarlanmıştır. PE ve PP taşıyıcılarla genel amaçlı ürünler için uygundur. Verimliliği artırır ve hammadde maliyetlerini düşürür.',
    image: '/Product/Navbar/CALCICHEM-110FRF.webp',
    index: 8,
    offsetDescY: '50px',
    offsetTextY: '50px',
    offsetY: '20px'
  ...
  { 
    ...
    path: '/tr/products/CALCICHEM-275-PM',
    description: '75% ultra ince mineral dolgu içeren polipropilen bazlı mineral masterbatch. Mükemmel dispersiyon ve yüksek mineral yüklemesi ile performansı artırır. BOPP, CPP ve OPP filmlerin ekstrüzyonu sırasında doğrudan ekleme için formüle edilmiştir.',
    image: '/Product/Navbar/CALCICHEM-275PM.webp',
    index: 9,
    offsetDescY: '70px',
    offsetTextY: '50px',
    offsetY: '20px'
  ...
  { 
    code: '', 
    title: 'UVCHEM MB-R18', 
    path: '/tr/products/UVCHEM-MB-R18',
    description: 'Rafya ve dokuma kumaşlar için yüksek performanslı UV stabilize edici masterbatch. HALS ve UV emiciler içerir; uzun vadeli foto-oksidasyona, renk solmasına ve mekanik özellik kaybına karşı direnç sağlar. Mükemmel dispersiyon, termal stabilite ve PP matrisi ile uyumluluk sunar.',
    image: '/Product/Navbar/UVCHEM-MBR18.webp',
    index: 10,
    offsetY: '24px',
    offsetTextY: '8px',
    offsetDescY: '22px'
...
// Helper function to check if path is a product page
const isProductPage = (path) => {
  if (!path) return false
  const normalizedPath = path.toLowerCase()
  return normalizedPath.includes('/tr/product') ||
         normalizedPath === '/tr/products' ||
         normalizedPath.includes('/tr/products/') ||
         normalizedPath.includes('/tr/product_polychem') ||
         normalizedPath === '/tr/product'
}
...
const openMenu = () => {
  ...
  // فقط در مرورگر body.style را تغییر بده
  if (process.client) {
...
const closeMenu = () => {
  ...
  tl.to(previewBox.value, {
    ...
    onComplete: () => {
      ...
      // پاک کردن event listeners با بررسی
      if (process.client && itemsWrapper.value) {
        try {
...
// Function to handle return from product page and open menu
const handleReturnFromProduct = async () => {
  // Activate button
  productActive.value = true
  
  // Check if we're on a main page
  const currentPath = route.path
  const isMainPage = ['/', '/about', '/news', '/careers', '/contact'].includes(currentPath)
  
  if (isMainPage && !isMenuOpen.value && !isOpeningMenu.value) {
    await nextTick()
    isMenuOpen.value = true
    await nextTick()
    try { hoveredItem.value = useCases[0] } catch (e) {}
...
onMounted(async () => {
  // فقط کدهای مربوط به DOM در onMounted اجرا شود
  if (!process.client) return
  
  await nextTick()
  
  // Initialize previous route
  previousRoute.value = { path: route.path, query: route.query }
  
  // اضافه کردن event listener برای dropdown
...
watch(() => route.fullPath, async (newFullPath, oldFullPath) => {
  // Get current and previous paths (without query)
  const newPath = route.path
  const oldPath = previousRoute.value?.path
  
  // Update previous route
  previousRoute.value = { path: route.path, query: route.query }
  
  // Only proceed if we have both paths
  if (!oldPath || !newPath) return
  
  // Check if we're coming FROM a product page TO a main page
  const comingFromProduct = isProductPage(oldPath)
  const goingToMainPage = ['/', '/about', '/news', '/careers', '/contact'].includes(newPath)
  
  // Only open menu in this specific case
  if (comingFromProduct && goingToMainPage) {
...
onUnmounted(() => {
  // فقط در مرورگر cleanup انجام بده
  if (!process.client) return
  ...
  // پاک کردن event listeners با بررسی امن
  if (itemsWrapper.value) {
    try {
      itemsWrapper.value.removeEventListener('wheel', wheelHandler)
      itemsWrapper.value.removeEventListener('touchmove', touchHandler)
  ...
  // پاک کردن dropdown listener
  try {
    window.removeEventListener('click', closeDropdownOutside)
  } catch (err) {
    console.error('Error removing dropdown listener:', err)
  }
})
...
@font-face {
  font-family: 'IRANYekan';
  src: url('/fonts/Qs_Iranyekan_bold.ttf') format('truetype');
  font-weight: 700;
  font-style: normal;
}
</style>